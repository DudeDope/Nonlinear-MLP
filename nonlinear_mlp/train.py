...
def compute_regularization(model, cfg):
    reg_loss = 0.0
    if cfg.approach == "gating":
        for layer in model.feature_layers:
            ...
            if cfg.gating.entropy_reg > 0:
                ...
                reg_loss += cfg.gating.entropy_reg * entropy.mean()
            if cfg.gating.l1_reg > 0:
                reg_loss += cfg.gating.l1_reg * a.abs().mean()
        if cfg.gating.sparsity_target is not None:
            ...
            reg_loss += cfg.gating.sparsity_loss_weight * diff.abs()
    return reg_loss

def train_one_epoch(model, loader, optimizer, device, scaler, epoch, cfg, criterion=nn.CrossEntropyLoss(), log_interval=100):
    model.train()
    ...
    for batch_idx, (x, y) in enumerate(loader):
        ...
        with autocast(enabled=cfg.training.amp and device.startswith("cuda")):
            logits = model(x)
            loss = criterion(logits, y)
            reg_loss = compute_regularization(model, cfg)
